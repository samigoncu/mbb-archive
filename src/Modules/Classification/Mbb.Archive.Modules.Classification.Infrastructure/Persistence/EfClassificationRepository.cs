...
internal sealed class EfClassificationRepository : IClassificationRepository
{
    ...
    public Task<bool> HasDocumentMetadataAsync(MetadataSchemaId schemaId, CancellationToken ct) => _db.DocumentMetadataSets.AnyAsync(x => x.SchemaId == schemaId, ct);
    public void RemoveSchema(MetadataSchema schema) => _db.MetadataSchemas.Remove(schema);
}
