...
{
    private readonly IServiceScopeFactory _scopeFactory;private readonly OpenSearchHttpClient _openSearch;private readonly SearchIndexerOptions _options;private readonly TimeProvider _time;private readonly ILogger<SearchIndexerBackgroundService> _logger;private readonly string _workerId;
    public SearchIndexerBackgroundService(IServiceScopeFactory scopeFactory,OpenSearchHttpClient openSearch,IOptions<SearchIndexerOptions> options,TimeProvider time,ILogger<SearchIndexerBackgroundService> logger){_scopeFactory=scopeFactory;_openSearch=openSearch;_options=options.Value;_time=time;_logger=logger;_workerId=$"{Environment.MachineName}:{Environment.ProcessId}:search-indexer:{Guid.CreateVersion7():N}";}
