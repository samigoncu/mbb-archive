...
public sealed class MarkSearchIndexedCommandHandler : ICommandHandler<MarkSearchIndexedCommand>
{
    ...
    public async Task<Result> Handle(MarkSearchIndexedCommand command, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var otherJobs = await _jobs.GetAwaitingIndexJobsByDocumentIdAsync(job.DocumentId, ct);
            foreach (var other in otherJobs)
            {
                if (other.Id != job.Id)
                {
                    try { other.MarkIndexed(command.OccurredAt); } catch { }
                }
            }
