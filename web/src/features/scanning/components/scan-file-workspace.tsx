...
import { Maximize2, X, FileText, ImageIcon, RotateCw, Scissors, Trash2, Upload, ZoomIn, ZoomOut } from "lucide-react";
...
export function ScanFileQueue({ pages, selectedId, disabled, maxUploadBytes, onSelect, onRotate, onRemove, onSeparator, onDetectSeparators }: {
  pages: ScannedPage[]; selectedId?: string; disabled: boolean; maxUploadBytes: number;
  onSelect: (index: number) => void; onRotate: (index: number) => void; onRemove: (index: number) => void; onSeparator: (index: number) => void; onDetectSeparators: () => void;
}) {
